...
func _notification(what: int) -> void:
	if what == NOTIFICATION_READY:
		v_scrollbar.connect("value_changed", self, "_on_h_value_changed")
		h_scrollbar.connect("value_changed", self, "_on_v_value_changed")
...
func _update_scrollbars() -> void:
	...
	var h_size: Vector2 = get_scrollbar_size(h_scrollbar)
	var v_size: Vector2 = get_scrollbar_size(v_scrollbar)
...
func get_scrollbar_size(scrollbar: ScrollBar) -> Vector2:
	if !scrollbar.visible: return Vector2.ZERO
	scrollbar.rect_size = Vector2.ZERO
	return scrollbar.rect_size
